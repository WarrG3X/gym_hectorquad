...
def configure_her(params,env=None):
    print("Configuring HER")
    if not env:
...
def configure_ddpg(dims, params, reuse=False, use_mpi=True, clip_return=True,env=None):
    print("Configuring DDPG")
    if not env:
        env = cached_make_env(params['make_env'])
    sample_her_transitions = configure_her(params,env=env)
