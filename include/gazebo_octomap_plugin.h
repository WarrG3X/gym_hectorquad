...
#include <ros/ros.h>
#include <gym_hectorquad/Octomap.h>
#include <sdf/sdf.hh>
#include <std_srvs/Empty.h>

namespace gazebo {

/// Imports from rotors_gazebo_plugin/common.h
...
bool getSdfParam(sdf::ElementPtr sdf, const std::string& name, T& param, const T& default_value, const bool& verbose =
  ...
  if (sdf->HasElement(name)) {
    ...
    param = default_value;
    if (verbose)
      gzerr << "[gym_hectorquad] Please specify a value for parameter \"" << name << "\".\n";
  }
  return false;
}
...
class OctomapFromGazeboWorld : public WorldPlugin {
  ...
  */
  void CreateOctomap(const gym_hectorquad::Octomap::Request& msg);
 ...
 private:
  ...
  bool ServiceCallback(gym_hectorquad::Octomap::Request& req,
                       gym_hectorquad::Octomap::Response& res);
};
